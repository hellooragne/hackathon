...
my $dbh = DBI->connect("DBI:mysql:database=freeswitch;host=localhost","root","hackathon") or die "$0: Couldn't connect to database: " . DBI->errstr;

# this is the standard location of the cdr-csv
my @LS  = `ls -1t Master.csv.*`;
foreach my $line (@LS) {
    chop($line);
    ...
    $ul->execute();
    $ul->finish;
    system("cat $line >> /FULL_Master.csv"); # we do this to maintain a single FULL file if needed
    unlink $line;
}
